...
CodeMirror.defineMode("plsql", function(_config, parserConfig) {
  ...
  function tokenBase(stream, state) {
	...
	// Это возможно переменная?
	else if (ch == ":") {
		if (!stream.eat(" ")) {
			stream.eatWhile(/[\w\.]/);
			return ret("comment", "string-2");
	  }
    }
	else if (ch == "L") {
		if (stream.eat("I")) 
    ...
    // is it a operator?
      ...
      if (keywords && keywords.propertyIsEnumerable(stream.current().toLowerCase())) return ret("keyword", "variable-2");
...
(function() {
  ...
  var cKeywords = "abort accept access add all alter and any array arraylen as asc assert assign at attributes audit " +
        ...
        "savepoint schema segment select Select separate session set share snapshot some space split sql start statement " +
