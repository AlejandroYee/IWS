...
refresh:function(b){var a=this.element,d=this.options,e=this.menu,k=this.checkboxContainer,l=[],h="",g=a.attr("id")||f++;a.find("option").each(function(a){var b=c(this),k=this.parentNode,e=this.innerHTML,b=this.title||b.text(),f=this.value;a="ui-multiselect-"+(this.id||g+"-option-"+a);var p=this.disabled,m=this.selected,q=["ui-corner-all"],r=(p?"ui-multiselect-disabled ":" ")+this.className,n;"OPTGROUP"===k.tagName&&(n=k.getAttribute("label"),-1===c.inArray(n,l)&&(h+='<li class="ui-multiselect-optgroup-label '+
k.className+'"><a href="#">'+n+"</a></li>",l.push(n)));p&&q.push("ui-state-disabled");m&&!d.multiple&&q.push("ui-state-active");h+='<li class="'+r+'">';h+='<label for="'+a+'" title="'+b+'" class="'+q.join(" ")+'">';h+='<input id="'+a+'" name="multiselect_'+g+'" type="'+(d.multiple?"checkbox":"radio")+'" value="'+f+'" title="'+b+'"';m&&(h+=' checked="checked"',h+=' aria-selected="true"');p&&(h+=' disabled="disabled"',h+=' aria-disabled="true"');h+=" /><span>"+e+"</span></label></li>"});k.html(h);this.labels=
...
a&&(a=b.noneSelectedText)):a=b.selectedText.replace("#",e).replace("#",a.length);this._setButtonValue(a);return a},_setButtonValue:function(b){var a=this.button.outerWidth(),c=!0;if(10*b.length>a)for(var e="",k=b.split(" "),l=0;l<k.length;l++)e=e+" "+k[l],20*e.length>a&&c&&(b=e+"...",c=!1);this.button.children(".ui-button-text").text(b)},_bindEvents:function(){function b(){a[a._isOpen?"close":"open"]();return!1}var a=this,d=this.button_arrow,e=this.button;d.find("span").bind("click.multiselect",b);
this.button.find("span").bind("click.multiselect",b);d.bind({click:b,keydown:function(b){switch(b.which){case 27:case 38:case 37:a.close();break;case 39:case 40:a.open()}},mouseenter:function(){d.hasClass("ui-state-disabled")||c(this).addClass("ui-state-hover")},mouseleave:function(){c(this).removeClass("ui-state-hover")},focus:function(){d.hasClass("ui-state-disabled")||c(this).addClass("ui-state-focus")},blur:function(){c(this).removeClass("ui-state-focus")}});e.bind({click:b,keydown:function(b){switch(b.which){case 27:case 38:case 37:a.close();
