...
'" type="text" '+e+' value="'+f[h[b].index]+'" />';break;case "checkbox":"1"===f[h[b].index]&&(e+='checked="checked"');a='<input class="FormElement ui-widget-content ui-corner-all" id="'+h[b].index+'" name="'+h[b].index+'" type="checkbox" '+e+" />";break;case "password":a='<input class="FormElement ui-widget-content ui-corner-all" id="'+h[b].index+'" name="'+h[b].index+'" type="password" '+e+' value="" />';break;case "textarea":a='<textarea class="FormElement ui-widget-content ui-corner-all" id="'+
h[b].index+'" '+e+' name="'+h[b].index+'" >'+f[h[b].index]+"</textarea>";break;case "select":a='<select class="FormElement ui-widget-content ui-corner-all" id="'+h[b].index+'" '+e+' name="'+h[b].index+'"  >',$.each(h[b].editoptions.value.split(";"),function(e,c){sel="";el=c.split(":");val=h[b].editoptions.multiple?f[h[b].index].split(","):f[h[b].index].split("~|");$.each(val,function(b,a){$.trim(a)===$.trim(el[0])&&(sel="selected")});a=a+'<option value="'+el[0]+'" '+sel+">"+el[1]+"</option>"}),a+=
...
l=$("<div />").attr({id:"colchooser_"+g[0].p.id,style:"position:relative;overflow-y:enable"}),k=$("<ol />").attr({style:"list-style-type: none; margin: 0; padding: 0; width: 100%;"});k.bind("mousedown",function(a){a.metaKey=!0}).selectable({selected:function(a,d){$(d.selected).addClass("ui-state-active")},unselected:function(a,d){$(d.unselected).removeClass("ui-state-active")}});$.each(a,function(a){this.hidedlg||(a=$("<li />").attr({value:a,"class":"ui-widget-content ui-state-default",style:"margin: 3px; padding: 0.4em; font-size: 0.8em; height: 12px; text-align:left; cursor:pointer;"}).text(f[a].replace("<br>",
...
!0===this.frozen&&!0===a.p.frozenColumns?a.grid.fhDiv:a.grid.hDiv).val(b),void 0!==b)e[c]=b,g++;else try{delete a.p.postData[c]}catch(k){}}});var h=0<g?!0:!1;if(!0===d.stringResult||"local"==a.p.datatype){var k='{"groupOp":"'+d.groupOp+'","rules":[',l=0;$.each(e,function(a,b){0<l&&(k+=",");k+='{"field":"'+a+'",';k+='"op":"eq",';k+='"data":"'+(b+"").replace(/\\/g,"\\\\").replace(/\"/g,'\\"')+'"}';l++});k+="]}";$.extend(a.p.postData,{filters:k});$.each(["searchField","searchString","searchOper"],function(b,
