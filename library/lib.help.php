				...
				foreach ($this -> main_menu as $key)
					if ($key['PARENT'] == $parent_id) {		
						if ((isset($key['MENU']) and ( $key['MENU'] > 0)) and ($key['PARENT'] == 0)) {
								if (is_file(iconv(HTML_ENCODING,LOCAL_ENCODING,ENGINE_ROOT."/".HELP_FOLDER."/".$key['ID']."-".$key['NAME'].".html"))) {
									...
									$res.= "<div id='".$key['ID']."'>\n";
									$first_parent = $key['ID'];
									$content =  str_replace("\n","<br>",file_get_contents(iconv(HTML_ENCODING,LOCAL_ENCODING,ENGINE_ROOT."/".HELP_FOLDER."/".$key['ID']."-".$key['NAME'].".html")));
									$content =  str_replace("%HELP_DIR%",ENGINE_HTTP."/".HELP_FOLDER."/",$content);	
									$res .= $content;
								} else if ($key['ID'] == 100000000000000) {
									$res.= "<h3><b>Основные принципы работы с системой</b></h3>\n<div id='mine'>\n";
									$content =  str_replace("\n","<br>",file_get_contents(iconv(HTML_ENCODING,LOCAL_ENCODING,ENGINE_ROOT."/".HELP_FOLDER."/-0-Основные принципы работы.html")));
									$content =  str_replace("%HELP_DIR%",ENGINE_HTTP."/".HELP_FOLDER."/",$content);	
									$first_parent = 'mine';
									$res .= $content;	
								}
								...
								// это разделы
								if (isset($key['NAME']) and is_file(iconv(HTML_ENCODING,LOCAL_ENCODING,ENGINE_ROOT."/".HELP_FOLDER."/".$key['ID']."-".$key['NAME'].".html"))) {
